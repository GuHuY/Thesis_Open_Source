...
def synthesize(position):
    ...
    #ecg:ECG数据, annotation:apnea注释数据, sf:采样频谱, name:数据title
    (ecg, annotation, sf, name) = Read_ECG_and_ANN(position)

    #更新当前信号采样频率
    samp_freq = sf

    # R: [时间ms, ECG电压值mv], A: [R峰时刻ms, 当前R峰到上一个R峰时间差ms]
    # R_Moment_List: A的横坐标(非ms, 表示R峰出现在第n个ECG电压值)
    (R, A, R_Moment_List) = R_A(ecg, samp_freq)
    # 求导, 用于评估间期变化是否异常
    b1 = B(A)
    
    # eva: 原封不动输出b1[1], refine_out: 修正后的R_Moment_List
    # 16/04/2019 为了加速获取数据集暂时关闭下一行并用eva = 0, refine_out = 0 代替
    (eva, refine_out) = R_refine1.refine(R[1], R_Moment_List, b1[1], samp_freq)
    # eva=0; refine_out=0

    # 16/04/2019 插值函数C()用了np.unique, 不需要使用Zero_Exception函数
    # A = Zero_Exception(A)
    a = smoothing(A, 3)
...
# 14/04/2019 加入samp_freq
def draw_plot(pack_list, period=[0, 1], samp_freq=100):
    """
    根据输入的pack_list绘图
