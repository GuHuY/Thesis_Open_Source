...
def synthesize(position):
    ...
    (ecg, qrs, annotation, sf, name) = read_record(position)
    samp_freq = sf
    qrs = R_Senior_Selection(qrs, ecg)

    (R, A) = R_A(ecg, qrs)
    #
    b1 = B(A)
    #
    (evaluation, ana) = R_refine.refine(R[1], qrs, b1[1], sf)
    # A = Zero_Exception(A)
    a = smoothing(A, 3)
    b = B(a)
    c = C(b)
    ...
    # return result, R, A
    return result, R, A, evaluation, qrs


# def image_test(R, A, plot):
def image_test(R, A, plot, E, F):
    """
    测试用
    """
    a1 = smoothing(A, 3)
    ...
    for item in plot[1]:
        ...
        if item == 'ana':
            pack_list.append([R, '', 'reconsider', 1, F])
...
def R_A(ecg, qrs):
    ...
    R = [np.linspace(0, int(len(ecg)*(1000/samp_freq)),
                     ...
                     endpoint=False),
         ecg]
    R_Interval_List = np.append(np.array(qrs[1:]), [0]) - np.array(qrs)
    A = [qrs*(1000/samp_freq), R_Interval_List*(1000/samp_freq)]
    return R, A
...
def B(B_in):
    ...
    for i in range(1, len(B_in[0])):
        Derivative.append(1-B_in[1][i-1]/B_in[1][i])
...
def R_Senior_Selection(RSS_in, ecg):
    """
    ...
    Parameters:
        RSS_in(np.array): R wave moment list
        ecg(np.array): ECG record.


    Return:
        (np.array)
    """
    inspection_range = int(0.05*samp_freq)
    for index_RML in range(1, len(RSS_in)):
        temp = RSS_in[index_RML]
        max_index = np.argmax(np.array([ecg[x] for x in
