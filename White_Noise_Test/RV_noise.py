...
for DataID in datalist:
    ...
    # 加入噪声
    RR_interval_noise_arr = wgn(RR_interval_arr, 20)

    # # 均值滤波
    After_mov_filter_list = Moving_Mean_Filter(RR_interval_noise_arr, 1)
    ...
    file_path = '/Users/rex/python/Thesis_Open_Source/White_Noise_Test/RV_noise/' + DataID + '.txt'
    path = Path(file_path)
    if path.exists():
        print('update ', end="")
