...
# 上标签
half_sample_size = 30
divisor = samp_freq * 60
QRS_list = list(QRS_arr)
samples_with_labels = []
for idx in range(half_sample_size, len(QRS_list)-half_sample_size):
    QRS_idx = QRS_list[idx]
    label = annotation[QRS_idx//divisor]
    sample = list(Indicator[idx-half_sample_size:idx+half_sample_size])
    sample.append(label)
    samples_with_labels.append(sample)


file_address = '/Users/rex/python/z_thesis/RR_trace/'
print(file_address)
...
for idx in range(0,60):
    my_fmt += ' %d'
print(my_fmt)
np.savetxt(file_address + DataID + '.txt', np.array(samples_with_labels), fmt=my_fmt)
my=1
pass
