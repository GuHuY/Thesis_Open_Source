    ...
    if in_trace:
        ...
        if trace_length<20:
            ...
            if abs(trace_x-trace_y) >= threshold :
                ...
                #轨迹结束
                trace_list.append([trace_head, posi, trace_length, height_sum/trace_length])
                in_trace = False
                stable = True
                posi = []
                trace_length = 0
        ...
        if stable:
            # 过去是稳态
            if abs(trace_x-trace_y) >= threshold: # interval_features_list 单位为10ms, 即误差80ms,小于PR间期
                #轨迹开始
                trace_head = idx #+ 1
                posi.append([trace_x, trace_y])
                trace_length += 1
...
for item in trace_list:
    my_x.append(QRS[item[0]])
    my_y.append(item[2]/10)
